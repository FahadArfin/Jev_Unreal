...
if ($AutomationTests) {
    ...
    if ([int]$report.failed -ne 0 -or [int]$report.notRun -ne 0 -or [int]$report.inProcess -ne 0 -or $passedCount -lt 3) { throw "Unreal automation failed or incomplete: passed=$passedCount, failed=$($report.failed)." }
    foreach ($expectedTest in @('Jev.Editor.PlanLifecycle', 'Jev.Editor.PlanSafety', 'Jev.Editor.SchemaSafety')) {
