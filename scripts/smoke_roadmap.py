...
async def main():
    ...
    async with stdio_client(params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            assert len(tools) == 44, tools
            status = await call(session, "unreal_status")
            assert status["bridge_version"] == "0.7.0"
            ...
            assert rules["instance_lifetime"] == "fresh_transient_instance_per_asset_rule"
            assert rules["post_asset_validation_called"] is False
            compile_targets = await call(session, "unreal_blueprint_compile_targets")
            assert compile_targets["enabled"] is False
            await call(
                session,
    ...
    evidence = {
        ...
        "default_blueprint_compile_policy_refused": True,
    ...
    destination = ROOT / "artifacts/roadmap-smoke-v0.7.json"
